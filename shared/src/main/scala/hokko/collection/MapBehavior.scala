...
class MapBehaviorOps[K, V, D[X, Y] <: MapBehavior.MapDiffLike[X, Y, D] with MapBehavior.MapDiff[X, Y]] private[collection] (
  ...
  private def selfPatch[B](evt: Event[B])(f: B => MapDiff[K, V]): MapBehavior[K, V, MapDiff] = {
    ...
    val merged = conses.unionWith(self.deltas)(identity)(x => x: MapDiff[K, V]) { (cons, delta) => Merged(delta, cons) }
...
object MapBehavior {
  def empty[K, V]: MapBehavior[K, V, Nothing] =
    fromMap(Map.empty[K, V])
  def fromMap[K, V, D[X, Y] <: MapDiffLike[X, Y, D] with MapDiff[X, Y]](set: Map[K, V]): MapBehavior[K, V, D] =
